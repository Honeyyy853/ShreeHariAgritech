...
const EditDehydratedVegetables = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    axios
      .post(`http://localhost/ShreeHari/getSingleDV.php`, formdata)
      .then((res) => {
        const json = res.data;
        if (json.status == "true") {
          var data = json.data;
  ...
  const updateDF = () => {
    ...
    axios
      .post("http://localhost/ShreeHari/updateDV.php", formData)
      .then((res) => {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="fixed top-20 right-4 z-50 bg-green-50 border border-green-200 rounded-lg p-4 flex gap-3 shadow">
          ...
          <div>
            <p className="font-semibold">
              Dehydrated Vegetables Updated Successfully
            </p>
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold">Edit Dehydrated Vegetables</h1>
      </div>
          ...
          <Card hover={false}>
            {imagePreview ? (
              ...
              <label className="flex flex-col items-center justify-center h-64 border-dashed border rounded cursor-pointer">
                <img
                  src={`http://localhost/ShreeHari/uploads/DehydratedVegetables/${oldimage}`}
                  className="w-full h-64 object-cover rounded"
                />
...
export default EditDehydratedVegetables;
